...
class HooksProcess {
  ...
  [Symbol.for('traversingNextUnCloseChildProcess')](nextIdx = 1) {
    const bashChildProcessArray = this.bashChildProcessArray;
    const setProcessAsStdout = this.[Symbol.for('setProcessAsStdout')];

    while (let nextChildProcess = bashChildProcessArray[nextIdx]) {
  ...
  [Symbol.for('setProcessAsStdout')](nextChildProcess) {
    ...
    for (let output of nextChildProcess.getOutputIterator()) {
      console.log(ouput);
    }
