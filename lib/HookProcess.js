...
class HookProcess {
  ...
  [Symbol.for('switchChildProcessOutput')](currentIdx = 0) {
    ...
    // @TODO: if current process `isStdout=false`, empty output cache.
  ...
  [Symbol.for('traversingNextUnCloseChildProcess')](nextIdx = 1) {
    ...
    const length = bashChildProcessArray.length;

    for (let nextChildProcess = bashChildProcessArray[nextIdx];
      nextIdx < length && nextChildProcess.isClose(); nextIdx = nextIdx + 1) {
  ...
  [Symbol.for('setProcessAsStdout')](nextChildProcess) {
    ...
    let outputIterator = nextChildProcess[Symbol.for('outputQueue')];
    //@FIXME: Iteratror not effective.
    // ouput caching results
    outputIterator.forEach((output) => {
  ...
  [Symbol.for('bindExitCallback')](rejectPromises = []) {
    ...
    // @TODO: Print exit process message.
