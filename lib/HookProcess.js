...
class HookProcess {
  ...
  [Symbol.for('initHookParams')]() {
    ...
    const hookParams = new HookParams({ hookName }).getParams();
  ...
  [Symbol.for('traversingPluginNames')](pluginNames = []) {
    ...
    co(function*() {
      const hookName = _this.getHookName();
      const hookParams = yield _this.getHookParams();
      const environment = _this.getEnvironment();

      const processArray = pluginNames.map((pluginName, idx, arr) => {
        const pluginChildProcessMap = _this[Symbol.for('pluginChildProcessMap')];
      ...
      const rejectPromises = processArray.map((pluginChildProcess, idx, arr) => {
        return pluginChildProcess.getErrorPromise();
      });

      const closePromises = processArray.map((pluginChildProcess, idx, arr) => {
        return pluginChildProcess.getClosePromise();
      });

      _this[Symbol.for('bindProcessCloseCallback')]();
      _this[Symbol.for('bindExitCallback')](rejectPromises);
      _this[Symbol.for('bindCloseCallback')](closePromises);
    });
  }
